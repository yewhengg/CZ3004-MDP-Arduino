...
void RpiTest(char command){
  char toSend = char(command + 1);
...
float KP_MR = 4;//4
float KI_MR = 0.277;
float KD_MR = 0.29;//.29149;

void goStraightOneTime()
{
  Serial.print("Left PID Output");
  Serial.println(Output_ML);
  Serial.print("Right PID Output");
  Serial.println(Output_MR);
  SPEEDL = 200; //250
  SPEEDR = 200;
  nowTest2 = micros();
  PIDCompute(KP_ML, KI_ML, KD_ML, KP_MR, KI_MR, KD_MR, Setpoint);
